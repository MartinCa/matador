...
\begin{tikzpicture}
	\begin{umlseqdiag}
		...
		\begin{umlcallself}[op={rent=rent()},padding=1]{field}
		\end{umlcallself}
		\begin{umlcall}[op=getKonto(),return=konto,padding=2]{field}{player}
		\end{umlcall}
		\begin{umlcall}[op=withdraw(rent),return=bool success]{field}{konto}
		\end{umlcall}
		\begin{umlfragment}[type=alt,label=owner]
			\begin{umlcall}[op=deposit(rent),dt=5]{field}{player}
			\end{umlcall}
		\end{umlfragment}
