...
\begin{tikzpicture}
	\begin{umlseqdiag}
		...
		\umlobject[x=9,y=-5.5,class=Board]{board}
		\umlobject[x=11.6,y=-5.5,class=Field,stereo=multi]{boardFields}
		\umlobject[x=14,y=-5.5,class=Konto,stereo=multi]{konto}
		...
		\begin{umlcallself}[op=oneRound(),dt=3,padding=1]{game}
			...
			\begin{umlcall}[op=rollDice(),dt=2,padding=0]{game}{baeger}
				...
				\end{umlcall}
			\end{umlcall}
			\begin{umlcall}[op=getSum(),return=int sum,padding=1.5]{game}{baeger}
				\begin{umlcall}[op=getFacevalue(), return=int,padding=1.5,dt=0.5]{baeger}{dice}
				\end{umlcall}
			\end{umlcall}
			\begin{umlcall}[op=getField(sum - 2), return=Field,padding=1.5,dt=9]{game}{board}
			\end{umlcall}
			\begin{umlfragment}[type=ref,label={optToBuy(Field, Player)}]
				\begin{umlcallself}[padding=0,dt=0]{game}
				\end{umlcallself}
			\end{umlfragment}
			...
			\begin{umlcallself}[op=endRoundChecks(),dt=0,padding=1]{game}
				...
				%\begin{umlcallself}[op=checkWinner(),dt=1.3,padding=1]{game}
					...
					%\end{umlcall}
					\begin{umlfragment}[type=alt,label=checkWinner(),inner xsep=8]
						\begin{umlcall}[op=showString(String),padding=0,dt=0]{game}{BoundaryToPlayer}
						\end{umlcall}
